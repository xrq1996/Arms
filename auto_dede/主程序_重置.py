...
def attack(i):
    while True:
        ...
        if not domain_model:
            ...
            continue
        status = 10
        for domains in domain_model:
            domain = domains[1]
            back_ground_url = domains[5]
            if 'http' not in back_ground_url and 'https' not in back_ground_url:
                back_ground_url = domain + '/' + back_ground_url
            ...
            if res["res"]:
                ...
                status = 10
            dbsqlite.data_update(domain, "status = %d,user = '%s'" % (status, res["info"]))
